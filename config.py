...
class StrategyConfig:
    ...
    # of waiting for the 1h candle to close past the level. DISABLED by default:
    # the edge is period-dependent. The first 30-day window favoured stop-touch
    # (PF 1.65 / +12.95$ vs limit-retrace PF 1.41 / +6.43$), but a later window
    # reversed it (stop-touch PF 0.87 / -2.03$ vs limit-retrace PF 1.42 / +6.54$).
    # Limit-retrace is the robust choice; stop-touch only wins when momentum is
    # clean. Asia BO is also off — its stop-touch model failed both windows.
