"""
ema_test.py — EMA trend filtreleri: squeeze'e EMA200 + EMA50/200 combo testi.
squeeze'in donchian'daki gibi bir EMA200 trend filtresi YOK (sadece 4h KC-mid MTF).
Ekleyince ters-trend squeeze breakout'ları elenir mi? Ayrıca EMA50>EMA200 rejim
(golden-cross) filtresi — hem squeeze hem donchian'da.

Filtreler (giriş timeframe'inde: squeeze 1h, donchian 4h):
  baseline    : filtre yok
  +EMA200     : long ancak close>EMA200, short ancak close<EMA200
  +EMA50/200  : long ancak EMA50>EMA200 (golden), short ancak EMA50<EMA200
  +HER İKİSİ  : ikisi birden

Kullanım:  py ema_test.py local
"""
import sys
import numpy as np, pandas as pd
import fast_bt
from indicators import atr as atr_fn, adx as adx_fn, ema as ema_fn
from strategies.donchian import DonchianStrategy
from strategies.squeeze import SqueezeStrategy

BAL = 190.0; FEE = 0.0001; RISK = 0.02
DEPLOY = {
    "donchian": (["SOL", "ETH", "ADA", "NEAR", "BCH"], "4h", 259, 2.0, 2.0, 30),
    "squeeze":  (["XRP", "DOGE", "TRX", "XLM"], "1h", 119, 2.0, 2.5, 48),
}


def gen(sleeve, coin, m):
    _, tf, win, sl_a, rr, mh = DEPLOY[sleeve]
    d = fast_bt.resample(m, tf)
    ema50 = ema_fn(d["close"], 50).values
    ema200 = ema_fn(d["close"], 200).values
    s = (DonchianStrategy(channel=40, rr=2.0, sl_atr=2.0, ema_trend=200, buffer_atr=0.0)
         if sleeve == "donchian" else
         SqueezeStrategy(kc_mult=1.5, min_squeeze_bars=5, sl_atr=2.0, rr=2.5, mtf_filter=True))
    hi = d["high"].values; lo = d["low"].values; cl = d["close"].values; idx = d.index; n = len(cl)
    out = []; occ = -1
    for i in range(260, n):
        sub = d.iloc[max(0, i - win):i + 1]
        a = atr_fn(sub["high"], sub["low"], sub["close"], 14).iloc[-1]
        if np.isnan(a) or a <= 0: continue
        adxv = adx_fn(sub["high"], sub["low"], sub["close"], 14).iloc[-1]
        adxv = float(adxv) if np.isfinite(adxv) else 20.0
        if sleeve == "squeeze" and adxv <= 20.0: continue
        sg = s.analyze(sub, float(a))
        if sg.direction == 0 or i <= occ or i >= n - 1: continue
        if np.isnan(ema200[i]) or np.isnan(ema50[i]): continue
        d_ = sg.direction; e = cl[i]; sld = sl_a * a; slp = e - d_ * sld; tp = e + d_ * rr * sld; ep = None; j = i
        for j in range(i + 1, min(i + 1 + mh, n)):
            if d_ == 1:
                if lo[j] <= slp: ep = slp; break
                if hi[j] >= tp: ep = tp; break
            else:
                if hi[j] >= slp: ep = slp; break
                if lo[j] <= tp: ep = tp; break
        if ep is None: j = min(i + mh, n - 1); ep = cl[j]
        R = d_ * (ep - e) / sld - 2 * FEE * e / sld
        over200 = (d_ == 1 and e > ema200[i]) or (d_ == -1 and e < ema200[i])
        golden = (d_ == 1 and ema50[i] > ema200[i]) or (d_ == -1 and ema50[i] < ema200[i])
        out.append({"R": R, "year": idx[i].year, "o200": over200, "gold": golden}); occ = j
    return out


def st(tr):
    if not tr: return "yok"
    r = np.array([t["R"] for t in tr]); gp = r[r > 0].sum(); gl = -r[r < 0].sum()
    pf = gp / gl if gl > 0 else 9.99
    return f"n={len(r):>4d} WR{(r>0).mean():>3.0%} PF{pf:4.2f} ${r.sum()*BAL*RISK:+8.2f}"


def yr_line(name, tr):
    yrs = sorted(set(t["year"] for t in tr))
    bits = []
    for y in yrs:
        r = np.array([t["R"] for t in tr if t["year"] == y])
        bits.append(f"{y}:${r.sum()*BAL*RISK:+.0f}")
    return "  " + " ".join(bits)


def main():
    source = sys.argv[1] if len(sys.argv) > 1 else "mexc_futures"
    for sleeve, (coins, *_) in DEPLOY.items():
        trs = []
        for coin in coins:
            try: m = fast_bt.load(coin, source=source)
            except Exception as e: print(f"  {coin}: {e}"); continue
            trs += gen(sleeve, coin, m)
        base = trs
        f200 = [t for t in trs if t["o200"]]
        fgold = [t for t in trs if t["gold"]]
        fboth = [t for t in trs if t["o200"] and t["gold"]]
        print(f"\n{'='*68}\n=== {sleeve.upper()} ===")
        print(f"  baseline   : {st(base)}")
        print(f"  +EMA200    : {st(f200)}")
        print(f"  +EMA50/200 : {st(fgold)}")
        print(f"  +HER İKİSİ : {st(fboth)}")
        # en iyi total olanın yıl-yıl'ı
        cands = {"baseline": base, "+EMA200": f200, "+EMA50/200": fgold, "+HER İKİSİ": fboth}
        best = max(cands, key=lambda k: np.array([t["R"] for t in cands[k]]).sum() if cands[k] else -9)
        print(f"  → en iyi total: {best}   yıl-yıl:{yr_line(best, cands[best])}")
        print(f"    baseline yıl-yıl:{yr_line('b', base)}")
    print("\n  Aranan: PF belirgin artıp total ~korunuyor VE her yıl iyi/eşit → uygula.")


if __name__ == "__main__":
    main()
