"""
maxhold_test.py — MAX-HOLD (tutma süresi) taraması. SL/RR sabit, max_hold değişir.
donchian 30 (4h=5g) / squeeze 48 (1h=2g) mevcut. Daha uzun/kısa net'i iyileştirir mi?
max_hold ÜRETİMDE (çıkış barı ona bağlı → occ doğru, canlı-doğru).

Kullanım:  py maxhold_test.py local
"""
import sys
...
DEPLOY = {
    "donchian": (["SOL", "ETH", "ADA", "NEAR", "BCH"], "4h", 259, 2.0, 2.0, 30, [15, 30, 45, 60, 90]),
    "squeeze":  (["XRP", "DOGE", "TRX", "XLM"], "1h", 119, 2.0, 2.5, 48, [24, 48, 72, 96, 120]),
}


def run(sleeve, coin, m, mh):
    _, tf, win, sl_a, rr, cur, _ = DEPLOY[sleeve]
...
def main():
    ...
    for sleeve, (coins, *rest) in DEPLOY.items():
        cur = rest[4]; mhs = rest[5]
        ms = {}
        for coin in coins:
            try: ms[coin] = fast_bt.load(coin, source=source)
            except Exception as e: print(f"  {coin}: {e}")
        print(f"\n{'='*66}\n=== {sleeve.upper()} — MAX-HOLD taraması ({tf_lbl(sleeve)}) ===")
        for mh in mhs:
            allt = []
            for coin, m in ms.items():
                allt += run(sleeve, coin, m, mh)
            mark = " ← MEVCUT" if mh == cur else ""
            print(f"  hold{mh:>3}: {st(allt)}{mark}")
    print("\n  Mevcut en iyiyse → dokunma. Belirgin daha iyi süre varsa → değerlendir.")


def tf_lbl(s):
    return "4h bar" if s == "donchian" else "1h bar"
